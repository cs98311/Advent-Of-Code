...
struct Hand {
  ...
  int tie_breaker{0};
  // modified strength : strength * tie_breaker
  int mod_strength{0};
  ...
  Hand(std::string letters, int bid) : bid(bid), hand_cards() {
    ...
    // Add 'J' 's count to the highest non-J count then erase 'J' from the map
    if (hand_cards.count('J') > 0) {
      // this initialization takes care of JJJJJ cases
      char max_value_key{'A'};
      int max_value{0};
      for (const auto &pair : hand_cards) {
        if (pair.first != 'J' && pair.second > max_value) {
          max_value = pair.second;
          max_value_key = pair.first;
        }
      }
      hand_cards[max_value_key] += hand_cards['J'];
      hand_cards.erase('J');
    }
    // 'K' : 5 or KKKKK
    ...
    // obtain modified strength by adding 16^5 * strength to the tie breaker
    mod_strength = strength * 16 * 16 * 16 * 16 * 16 + tie_breaker;
  }
};

// function to sort vector of vectors acc to index of inner vectors
void sort_vector(std::vector<std::vector<int>> *vec, size_t index) {
...
int main() {
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (line == "" || line == " ") {
      ...
      // matrix
      std::vector<int> temp;
      temp.push_back(hand.mod_strength);
      temp.push_back(hand.bid);
      ranks.push_back(temp);
    }
  }

  // sort the matrix acc to modified strength in ascending order
  sort_vector(&ranks, 0);

  // rank is index+1 since ascending order
  unsigned long int total_winnings{0};
  for (size_t i = 0; i < ranks.size(); i++) {
    total_winnings += static_cast<unsigned int>(ranks[i][1]) * (i + 1);
  }
