...
int main() {
  ...
  // no. of galaxies in each row, column
  ...
  unsigned long int expansion{1000000 - 1};

  // Get the empty rows in a vector
  ...
  for (auto i = 0UL; i < empty_columns.size(); i++) {
    for (auto j = 0UL; j < galaxy_coordinates.size(); j++) {
      if (galaxy_coordinates[j][1] > empty_columns[i]) {
        galaxy_coordinates[j][1] += expansion;
      }
    }
  }
